...
class Menu_public extends Backend{
  ...
  {
    parent::__construct();
    $this->load->model('Menu_public_model','model');
  }

  function _rules()
  {
     $this->form_validation->set_rules("name","Nama Menu","trim|required|xss_clean");
     $this->form_validation->set_rules("type","Target","trim|xss_clean");
  ...
  function index()
  {
    ...
    $this->temp_backend->view('content/menu_public/index',$data);
  }
  ...
  function save()
 {
   ...
   $readbleArray = $this->parseJsonArray($data);
     ...
     foreach($readbleArray as $row){
       $i++;
       $this->db->query("update $this->table set is_parent = '".$row['parentID']."', sort = '".$i."' where id = '".$row['id']."' ");
     }
   }
...
function tambah($status="")
{
  if ($status=="aksi") {
    ...
    $data = array('button' => "tambah",
                  'action'   => site_url("backend/menu_public/tambah/aksi"),
                  'name' => set_value("name"),
                  'type' => set_value("type"),
                  'url' => set_value("url"),
    ...
    $this->temp_backend->view('content/menu_public/form',$data);
  }
}
  ...
  if ($status=="aksi") {
    ...
    if ($row = $this->model->get_where($this->table,['id'=>$id])) {
      ...
      $data = array('button' => "edit",
                    'action' => site_url("backend/menu_public/edit/$id/aksi"),
