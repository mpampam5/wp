        ...
        <div class="card">
            <div class="card-header">
              ...
              <div style="position:absolute;right:10px;" >
                ...
                <a href="<?=site_url('backend/menus/tambah')?>" class="btn btn-sm btn-primary btn-xs btn-icon-right">Tambah <i class="fa fa-plus"></i></a>
              </div>
            </div>
                    ...
                    foreach($items as $key=>$value) {
                      if ($value['active']==1) {
                        ...
                        $html.= '<li class="dd-item dd3-item" data-id="'.$value['id'].'" >
                                    ...
                                    <div class="dd3-content"><span id="label_show'.$value['id'].'"><i class="fa '.$value['icon'].'"></i>&nbsp;'.strtoupper($value['label']).'</span>
                                      <span class="span-right">
                                        <a class="buttons links"><i id="link_show'.$value['id'].'">'.strtolower(site_url('backend/'.$value['link'])).'</i></a>
                                        '.$active.'
                                        <a href="'.site_url('backend/menus/edit/'.$value['id']).'" class="buttons edit-button" data-toggle="tooltip" data-placement="bottom" title="Edit"><i class="fa fa-pencil"></i></a>
                                        <a href="'.site_url('backend/menus/hapus/'.$value['id']).'" id="delete" alt="'.$value['id'].'" class="buttons del-button" data-toggle="tooltip" data-placement="bottom" title="Hapus"><i class="fa fa-trash"></i></a>&nbsp;
                                      </span>
                                    </div>';
...
$(document).ready(function()
{
    ...
    $('.dd').on('change', function() {
       ...
       $.ajax({
           type: "POST",
           url: "<?=base_url()?>backend/menus/save",
           data: dataString,
           cache : false,
